...
//左soft 右Widgets
  void navigateToPages(BuildContext context, Item item, {bool isapp = false}) {
    switch (item.textheme) {
      case 'defaults':
        navigateToPage(context,
            isapp ? ImagePagesoft(item: item) : ImagePages(item: item), item);
        break;
      case 'default':
      ...
      case 'electron':
        navigateToPage(context,
            isapp ? Electronsoft(item: item) : MyHomePagebak(item: item), item);
        break;
      case 'lanbai':
        navigateToPage(
            context,
            isapp ? lanbaisoft(item: item) : AnniversaryPages(item: item),
            item);
        break;
      case 'shaolou':
        navigateToPage(
      ...
      case 'huanrao':
        navigateToPage(
            context,
            isapp
                ? AltAKeyListenersoft(item: item)
                : AltAKeyListener(item: item),
            item);
        break;

      case 'huaban':
        navigateToPage(context, isapp ? Papersoft() : Paper(item: item), item);
        break;
      default:
        // do nothing
        break;
...
//navigateToPages(context,  item);
          ...
          const Column(
            children: [
              SizedBox(
                width: 300,
                child: Text('为什么会一直停留在此页面?'),
              ),
              SizedBox(
                width: 400,
                child:
                    Text("1.此页面用于过渡:主程序与小部件之间互相转换,\n3.若过渡窗体页面过大,则过渡失败,请重新过渡"),
              ),
              SizedBox(
                width: 400,
                child: Text('2.所选主题不支持小部件,或选择了未适配的主题'),
              ),
              SizedBox(
                width: 400,
                child: Text("4.未设置小部件属性,或错误的设置了小部件属性"),
              ),
              SizedBox(
                width: 400,
                child: Text('* 5.由于不同状态之间切换布局会造成布局混乱,此页作为缓冲区(防止窗口渲染失败导致的内存溢出)'),
              ),
            ],
          ),
